...
void InitPassShadowPass( const defParms_shadow &info, CBaseVSShader *pShader, IMaterialVar **params )
{
	...
	if ( PARM_DEFINED( info.iAlbedo3 ) ) pShader->LoadTexture( info.iAlbedo3 );
	if ( PARM_DEFINED( info.iAlbedo4 ) ) pShader->LoadTexture( info.iAlbedo4 );
}
	...
	VertexCompressionType_t vertexCompression, CDeferredPerMaterialContextData *pDeferredContext )
{
	...
	SHADOW_STATE
	{
		...
		if ( bNoCull ) pShaderShadow->EnableCulling( false );
		...
		if ( bModel ) iVFmtFlags |= VERTEX_FORMAT_COMPRESSED;
	...
	DYNAMIC_STATE
	{
...
#if DEFCFG_ENABLE_RADIOSITY
			if ( bAlbedo ) tmpBuf.BindTexture( pShader, SHADER_SAMPLER0, info.iAlbedo );
			else tmpBuf.BindStandardTexture( SHADER_SAMPLER0, TEXTURE_WHITE );
			if ( bBaseTexture2 || bMultiBlend ) tmpBuf.BindTexture( pShader, SHADER_SAMPLER1, info.iAlbedo2 );
			if ( bMultiBlend )
			{
				tmpBuf.BindTexture( pShader, SHADER_SAMPLER2, info.iAlbedo3 );
				if ( bAlbedo4 ) tmpBuf.BindTexture( pShader, SHADER_SAMPLER3, info.iAlbedo4 );
				else tmpBuf.BindStandardTexture( SHADER_SAMPLER3, TEXTURE_WHITE );
			}
#endif
			tmpBuf.End();
		...
		if ( radiosityOutput )
		{
			...
			pShaderAPI->SetPixelShaderConstant( 1, globalLight.vecLight.AsVector3D().Base() );
			pShaderAPI->SetPixelShaderConstant( 2, globalLight.diff.AsVector3D().Base() );
		}
#endif
